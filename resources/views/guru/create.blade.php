            ...
            <div class="container mt-3">
                ...
                <form action="{{ route('guru.store') }}" method="POST" class="needs-validation" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('id_guru') is-invalid @enderror" id="id_guru" name="id_guru" value="{{ old('id_guru') }}" required maxlength="15">
                        ...
                        <select class="form-select @error('id_jurusan') is-invalid @enderror" id="id_jurusan" name="id_jurusan" required>
                           <option value="">Pilih</option>
                            @foreach ($jurusan as $item)
                                <option value="{{ $item->id_jurusan }}" {{ old('id_jurusan') == $item->id_jurusan ? 'selected' : '' }}>{{ $item->jurusan }}</option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">
