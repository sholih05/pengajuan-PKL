        ...
        <form action="{{ route('penilaian.update', $siswa->id_siswa) }}" method="POST" id="penilaianForm">
                                ...
                                <td>
                                    @if($indikator->is_nilai == '1')
                                        <input type="number" class="form-control nilai-input" 
                                            ...
                                            value="{{ $nilaiArray[$indikator->id] ?? '' }}">
                                    @else
                                        <span class="badge bg-secondary">Tidak Dinilai</span>
                                    @endif
                                </td>
                                <td>
                                    <span class="keterangan" id="ket-{{ $indikator->id }}">
                                        {{ isset($nilaiArray[$indikator->id]) ? getNilaiKeterangan($nilaiArray[$indikator->id]) : '-' }}
                                    </span>
                                </td>
                            </tr>
                                        ...
                                        <td>
                                            @if($child->is_nilai == '1')
                                                <input type="number" class="form-control nilai-input" 
                                                    ...
                                                    value="{{ $nilaiArray[$child->id] ?? '' }}">
                                            @else
                                                <span class="badge bg-secondary">Tidak Dinilai</span>
                                            @endif
                                        </td>
                                        <td>
                                            <span class="keterangan" id="ket-{{ $child->id }}">
                                                {{ isset($nilaiArray[$child->id]) ? getNilaiKeterangan($nilaiArray[$child->id]) : '-' }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
            ...
            <div class="row mb-3 mt-4">
                <div class="col-md-12">
                    ...
                    <textarea class="form-control" id="catatan" name="catatan" rows="4">{{ $catatanText }}</textarea>
                </div>
            </div>
